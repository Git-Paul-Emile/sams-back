...
function isoWeekLabel({ isoYear, week }) {
...
async function computePeriode(start, end) {
    ...
    for (const group of factureModeGroups) {
        ...
        if (group.mode === "—")
            continue; // mode de règlement non renseigné
        if (CASH_MODES.has(group.mode))
            cash += amount;
        else
            depot += amount;
    }
...
export const financeService = {
    ...
    async getVentesCommercial() {
        ...
        const margeByCommercialId = new Map();
