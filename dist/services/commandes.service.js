...
export const commandesService = {
    ...
    async get(id) {
        ...
        if (!commande)
            throw new AppError("Commande introuvable", StatusCodes.NOT_FOUND);
        return toCommandeDto(commande);
    },
    async create(input, ctx) {
        const now = new Date();
        const created = await createWithSequenceNumber("CMD", (likePrefix) => commandesRepository.countByNumPrefix(likePrefix), (num) => commandesRepository.createWithTimeline({
            num,
    ...
    async addTimelineEvent(id, input, ctx) {
