...
export const matieresPrixService = {
    async list() {
        ...
        const produitsByMatiereId = await matieresPrixRepository.produitDesignationsByMatiereIds(rows.map((row) => row.matiereId));
        return rows.map((row) => toMatierePrixDto(row, produitsByMatiereId.get(row.matiereId) ?? []));
    },
};
//# sourceMappingURL=matieresPrix.service.js.map