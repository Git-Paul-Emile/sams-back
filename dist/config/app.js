...
app.use((err, req, res, next) => {
    ...
    if ("code" in err && typeof err.code === "string") {
        switch (err.code) {
            case "P2002":
                return res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "error", message: `Conflit unique sur le champ: ${err.meta?.target || "inconnu"}` }));
            case "P2025":
                return res.status(StatusCodes.NOT_FOUND).json(jsonResponse({ status: "not_found", message: "Ressource non trouvée" }));
            default:
                return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(jsonResponse({ status: "error", message: "Erreur de base de données inconnue" }));
        }
    }
