 ...
 */
export function currentYYMM(now = new Date()) {
    const yy = String(now.getFullYear()).slice(-2);
    const mm = String(now.getMonth() + 1).padStart(2, "0");
    return `${yy}${mm}`;
}
export async function nextSequenceNumber(prefix, countExisting, now = new Date()) {
...
function isNumUniqueConflict(err) {
    ...
    if (Array.isArray(classicTarget) && classicTarget.includes("num"))
        return true;
 ...
 */
export async function createWithSequenceNumber(prefix, countExisting, createFn, maxAttempts = 5) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            if (!isNumUniqueConflict(err) || attempt === maxAttempts)
                throw err;
        }
    }
