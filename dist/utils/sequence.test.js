...
function pgUniqueConflict(field) {
    const err = { code: "P2002" };
    err.meta = { driverAdapterError: { cause: { constraint: { fields: [field] } } } };
    return err;
}
function classicUniqueConflict(field) {
    const err = { code: "P2002", meta: { target: [field] } };
    return err;
}
...
describe("createWithSequenceNumber", () => {
    ...
    it("propage immédiatement une erreur qui n'est pas un conflit sur `num`", async () => {
        ...
        await expect(createWithSequenceNumber("CMD", countExisting, createFn)).rejects.toThrow("Client introuvable");
        expect(createFn).toHaveBeenCalledTimes(1);
    });
