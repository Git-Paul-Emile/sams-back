...
export const commerciauxRepository = {
  findMany(
    ...
    opts: { skip: number; take: number; orderBy: Prisma.CommercialOrderByWithRelationInput }
  ) {
    return prisma.commercial.findMany({ where, ...opts });
  },

  count(where: Prisma.CommercialWhereInput) {
  ...
  create(data: Prisma.CommercialUncheckedCreateInput) {
    return prisma.commercial.create({ data });
  },

  update(id: string, data: Prisma.CommercialUncheckedUpdateInput) {
   ...
   */
  async aggregatesForCommerciaux(commercialIds: string[]) {
    if (commercialIds.length === 0) {
      return new Map<string, { caRealise: number; nbClients: number; nbCmd: number }>();
    }
    ...
    const result = new Map<string, { caRealise: number; nbClients: number; nbCmd: number }>();
    for (const id of commercialIds) result.set(id, { caRealise: 0, nbClients: 0, nbCmd: 0 });
    ...
    for (const group of clientGroups) {
      if (!group.commercialId) continue;
      const entry = result.get(group.commercialId);
      if (entry) entry.nbClients = group._count._all;
    }
