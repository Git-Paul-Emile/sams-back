...
export const stockItemsService = {
  ...
  async setStock(id: string, input: SetStockItemStockInput, ctx: ActionContext) {
    ...
    if (!existing) throw new AppError("Article de stock introuvable", StatusCodes.NOT_FOUND);
    if (input.stock < 0) throw new AppError("Le stock ne peut pas être négatif", StatusCodes.BAD_REQUEST);
