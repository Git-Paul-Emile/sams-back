...
import type {
  ...
  ListFournisseursQuery,
  UpdateFournisseurInput,
} from "../validators/fournisseurs.validator.js";
...
export const fournisseursService = {
  async list(query: ListFournisseursQuery) {
    const { page, pageSize, skip, take } = parsePagination(query);
    const where: Prisma.FournisseurWhereInput = {
      ...(query.search ? { nom: { contains: query.search, mode: "insensitive" } } : {}),
    };
    const orderBy: Prisma.FournisseurOrderByWithRelationInput = { [query.sort ?? "nom"]: query.order ?? "asc" };
  ...
  async get(id: string) {
    ...
    if (!fournisseur) throw new AppError("Fournisseur introuvable", StatusCodes.NOT_FOUND);
    return toFournisseurDto(fournisseur);
  },

  async create(input: CreateFournisseurInput, ctx: ActionContext) {
  ...
  async update(id: string, input: UpdateFournisseurInput, ctx: ActionContext) {
    const existing = await fournisseursRepository.findById(id);
    if (!existing) throw new AppError("Fournisseur introuvable", StatusCodes.NOT_FOUND);
