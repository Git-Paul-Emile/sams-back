...
export const salesRequestsService = {
  ...
  async get(id: string) {
    ...
    if (!sr) throw new AppError("Demande de sortie introuvable", StatusCodes.NOT_FOUND);
    return toSalesRequestDto(sr);
  },

  async create(input: CreateSalesRequestInput, ctx: ActionContext) {
    const produit = await prisma.stockItem.findUnique({ where: { id: input.produitId } });
    if (!produit) throw new AppError("Produit introuvable", StatusCodes.NOT_FOUND);

    const created = await createWithSequenceNumber(
      "SSV",
      (likePrefix) => salesRequestsRepository.countThisMonth(likePrefix),
      (num) =>
        salesRequestsRepository.create({
          num,
  ...
  async approve(id: string, input: ApproveSalesRequestInput, ctx: ActionContext) {
    const result = await prisma.$transaction(async (tx) => {
      const sr = await tx.salesRequest.findUnique({ where: { id }, include: { produit: true } });
      if (!sr) throw new AppError("Demande de sortie introuvable", StatusCodes.NOT_FOUND);
      if (sr.statut !== PENDING_STATUT) throw new AppError("Cette demande a déjà été traitée", StatusCodes.CONFLICT);
  ...
  async reject(id: string, input: RejectSalesRequestInput, ctx: ActionContext) {
    const result = await prisma.$transaction(async (tx) => {
