...
vi.mock("../repositories/auth.repository.js", () => ({
  authRepository: {
    ...
    findModulesForRole: (...args: unknown[]) => findModulesForRole(...args),
    touchLastLogin: (...args: unknown[]) => touchLastLogin(...args),
...
describe("authService.login", () => {
  ...
  it("rejette un email inconnu", async () => {
    ...
    await expect(authService.login("inconnu@sams.sn", "peu importe")).rejects.toThrow(
  ...
  it("rejette un mot de passe incorrect", async () => {
    ...
    await expect(authService.login("test@sams.sn", "MauvaisMotDePasse")).rejects.toThrow(
      "Identifiant ou mot de passe incorrect"
    );
  });
  ...
  it("rejette un compte désactivé même avec le bon mot de passe", async () => {
    ...
    await expect(authService.login("test@sams.sn", "BonMotDePasse123!")).rejects.toThrow(
      "Compte désactivé"
    );
  });
