...
import type {
  ...
  RejectMaterialRequestInput,
} from "../validators/materialRequests.validator.js";
...
export const materialRequestsService = {
  ...
  async get(id: string) {
    ...
    if (!mr) throw new AppError("Demande de matières introuvable", StatusCodes.NOT_FOUND);
    return toMaterialRequestDto(mr);
  },

  async create(input: CreateMaterialRequestInput, ctx: ActionContext) {
    const prodOrder = await prisma.prodOrder.findUnique({ where: { id: input.prodOrderId } });
    if (!prodOrder) throw new AppError("Ordre de fabrication introuvable", StatusCodes.NOT_FOUND);
    const matiere = await prisma.stockItem.findUnique({ where: { id: input.matiereId } });
    if (!matiere) throw new AppError("Matière introuvable", StatusCodes.NOT_FOUND);

    const created = await createWithSequenceNumber(
      "SMR",
      (likePrefix) => materialRequestsRepository.countThisMonth(likePrefix),
      (num) =>
        materialRequestsRepository.create({
          num,
  ...
  async approve(id: string, input: ApproveMaterialRequestInput, ctx: ActionContext) {
    const result = await prisma.$transaction(async (tx) => {
      const mr = await tx.materialRequest.findUnique({ where: { id }, include: { matiere: true, prodOrder: true } });
      if (!mr) throw new AppError("Demande de matières introuvable", StatusCodes.NOT_FOUND);
      if (mr.statut !== PENDING_STATUT) throw new AppError("Cette demande a déjà été traitée", StatusCodes.CONFLICT);
  ...
  async reject(id: string, input: RejectMaterialRequestInput, ctx: ActionContext) {
    const result = await prisma.$transaction(async (tx) => {
